...
class ProbabilisticModel:
    ...
    def generate_structure(cls, seed=0):
        ...
        continuous_node_types = np.asarray(
        ...
        bn = pbn.SemiparametricBN(
            discrete_nodes + continuous_nodes,
            [(n, pbn.DiscreteFactorType()) for n in discrete_nodes]
        ...
        arcs = np.asarray(
            [
                (s, d)
                for i, s in enumerate(continuous_nodes[:-1])
                for d in continuous_nodes[i + 1 :]
            ]
        )
        num_arcs = arcs.shape[0]
        active_arcs_index = np.random.choice(
    ...
    def generate_clg_parameters(cls, structure, node):
        ...
        discrete_evidence = list(
            set(parents).intersection(set(ProbabilisticModel.discrete_nodes))
        )
        continuous_evidence = list(
            set(parents).intersection(set(ProbabilisticModel.continuous_nodes))
        )
    ...
    def generate_ckde_parameters(cls, structure, node):
        ...
        return NormalMixtureCPD.new_random_cpd(
    ...
    def generate_parameters(cls, structure, seed=0):
        ...
        for node in structure.nodes():
            ...
            if node_type == pbn.DiscreteFactorType():
                cpds.append(
                    ProbabilisticModel.generate_discrete_parameters(structure, node)
                )
                ...
                cpds.append(
                    ProbabilisticModel.generate_ckde_parameters(structure, node)
                )
